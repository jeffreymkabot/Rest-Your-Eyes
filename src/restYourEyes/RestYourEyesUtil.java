...
/**
* This class represents an application that repeatedly reminds the user
* to rest their eyes on a configurable interval.
*
* @author Sean Pesce
*/
public class RestYourEyesUtil extends Application {
	...
	public static boolean useDarkTheme = DEFAULT_USE_DARK_THEME;
	...
	public static final Alert reminder = new Alert(Alert.AlertType.NONE, "Be sure to take a break from screen time to avoid eye strain!");
	public static final HBox reminderBtPane = new HBox();
	public static final Button disableRemindersBt = new Button("Disable reminders");
		...
		try {
			...
			while(fileReader.hasNextLine()){
				...
				if(isConfigKeyValue(line, STARTUP_HIDE_KEY)){
					...
					try{
						long newInterval = Long.parseLong(getConfigKeyValue(line, INTERVAL_KEY));
    	...
    	javax.swing.SwingUtilities.invokeLater(new Runnable() {
	        ...
	        public void run() {
		        ...
		        try {
		            ...
		            trayIcon.addActionListener(event -> javafx.application.Platform.runLater(new Runnable() {
						@Override
						public void run() {
							stage.show();
						}
		            ...
		            openItem.addActionListener(event -> javafx.application.Platform.runLater(new Runnable() {
						@Override
						public void run() {
							stage.show();
						}
	...
	/**
     * Initialize reminder dialog window
     */
	...
	/**
     * Initialize handler functions to respond to user interaction with the GUI
     */
	public static void initEventHandlers(){
		
		fileToolbar.heightProperty().addListener(new ChangeListener<Number>() {
            @Override
		...
		newIntervalField.textProperty().addListener(new ChangeListener<String>() {
            @Override
	...
	public static void updateElements(){
		...
		Stage reminderStage = ((Stage)reminder.getDialogPane().getScene().getWindow());
		...
		javafx.application.Platform.runLater(new Runnable() {
			...
			public void run() {
				...
				if(useDarkTheme){
					...
					if(!reminderStage.getScene().getStylesheets().contains(DARK_STYLE_SHEET))
						reminderStage.getScene().getStylesheets().add(DARK_STYLE_SHEET);
					...
					if(reminderStage.getScene().getStylesheets().contains(DARK_STYLE_SHEET))
						reminderStage.getScene().getStylesheets().remove(DARK_STYLE_SHEET);
	...
	/**
     * @return the substring of the given line that follows the given key string
     */
